...
function renderGeneratedModule(icons: ReadonlyArray<[IconName, GeneratedIconData]>): string {
  const entries = icons
    .map(([name, icon]) => {
      ...
      return `  ${JSON.stringify(name)}: { render: () => (<>${jsxBody}</>), viewBox: ${JSON.stringify(icon.viewBox)} },`;
    })
    .join('\n');
  ...
  return (
    ...
    `export const generatedIcons: Record<IconName, GeneratedIcon> = {\n${entries}\n};\n`
  );
}
