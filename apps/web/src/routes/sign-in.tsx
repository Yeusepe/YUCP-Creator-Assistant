...
function SignInPage() {
  ...
  useEffect(() => {
    setSignInUrl(
      `/api/auth/sign-in/discord?callbackURL=${encodeURIComponent(window.location.href.split('?')[0])}`
    );
  }, []);
        ...
        <div className="card">
          ...
          {currentState === 'state-signin' && (
            <div id="state-signin" className="state active">
              ...
              <a id="discord-signin-btn" href={signInUrl} className="discord-btn" role="button">
                <svg
                  width="20"
                  height="15"
                  viewBox="0 0 22 16"
