...
export const createSchema = async ({
  ...
  const relativeDir =
    projectRoot !== null
      ...
      : null;
  const commandOutputPath = file
    ? path.relative(cwd, path.resolve(cwd, file)).split(path.sep).join("/")
    : null;
  const normalizedCommandOutputPath =
    commandOutputPath && !commandOutputPath.startsWith(".")
      ? `./${commandOutputPath}`
      : commandOutputPath;
  const generateCommand = `bunx auth@1.7.0-rc.2 generate${
    normalizedCommandOutputPath
      ? ` --output ${JSON.stringify(normalizedCommandOutputPath)}`
      : ""
  }`;
