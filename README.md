...
- External integration contract gate: `bun run test:external-integrations`. This is the fast PR-facing slice for provider/runtime/API/consumer hardening. It explicitly covers the production-incident surfaces in `ops/production-regression-loop.ts`: provider contracts, identity boundaries, verification flows, account surfaces, and backfill paths. Its web consumer slice loads the shared licensed `Icon` component, so the root command enters the shared icon runner and requires the same read-only asset inputs as other web build and test entrypoints. GitHub supplies those inputs only to same-repository runs and visibly skips this licensed job for fork pull requests.
...
- Cloudflare Workers Builds must run from the repository root with automatic dependency installation disabled. In the Worker dashboard under `Settings > Build`, set `SKIP_DEPENDENCY_INSTALL=true`, `BUN_VERSION=1.3.9`, `HEROUI_AUTH_TOKEN`, `ASSETS_S3_BUCKET`, `ASSETS_S3_ENDPOINT`, and `ASSETS_S3_REGION` in `Build Variables and Secrets`. Add `ASSETS_S3_READONLY_ACCESS_KEY_ID` and `ASSETS_S3_READONLY_SECRET_ACCESS_KEY` as encrypted build secrets using a bucket-scoped read-only credential. Leave the build command empty, set the production deploy command to `bun run cloudflare:worker:deploy`, and set the non-production branch deploy command to `bun run cloudflare:worker:version:upload`. Cloudflare exposes these build inputs to the install, icon-generation, and deployment commands; runtime Worker secrets in `Settings > Variables & Secrets` are separate and are not available during the build. See Cloudflare's [Workers Builds configuration](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/).
