...
| `CDNGINE_DOCKER_REF`                         | dev      | Optional `Yeusepe/cdngine` git ref for the dev supervisor Docker Compose runtime |
...
- To have the dev supervisor launch `cdngine`, either set `CDNGINE_DIR` to an existing checkout for source-mode development or set `CDNGINE_DOCKER_REF` to a ref in `https://github.com/Yeusepe/cdngine`. The Docker path is the default automatic path: `bun run ops/cdngine-dev.ts` fetches CDNgine's no-checkout remote Compose file, pins its Docker build context to `CDNGINE_DOCKER_REF`, writes the generated Compose file under `.volumes/cdngine/docker`, and starts the `cdngine-runtime` service through Docker Compose. The default startup mode is `CDNGINE_START_MODE=server`, which starts the local platform and the long-lived CDNgine public runtime without the demo frontend. Set `CDNGINE_START_MODE=demo` to include the demo profile. When any CDNgine runtime is configured and no explicit CDNgine API vars are present, local dev injects `CDNGINE_API_BASE_URL=http://localhost:4000` and a local runtime bearer token for the Gumroad API process. Production must provide `CDNGINE_API_BASE_URL`/`CDNGINE_ACCESS_TOKEN` through Infisical. Both `bun run dev` and `bun run dev:infisical` print whether `cdngine` is enabled and which mode it will use, and if `CDNGINE_DIR` is set to a bad path they fail immediately instead of silently skipping `cdngine` before the supervisor starts.
